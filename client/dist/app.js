...
angular.module('app')
    .controller('ExperimentRunnerCtrl', function($scope, $rootScope, $http, benchmarkData) {
        ...
        $scope.sendRunCommand = function () {
            ...
            $http.post('http://localhost:8080/performance/run', $scope.data)
                .success(function (response) {
                    var taskId = response.result[1].id,
                        intervalId = setInterval(pollTaskStatus, 2000);

                    function pollTaskStatus() {
                        jQuery.get('http://localhost:8080/tasks/' + taskId)
                            .success(function (response) {
        ...
        return {
            link: function (scope, element, attr) {
                ...
                function addExecutionByNumber() {
                    $http.get("http://localhost:8080/instrumentations/756271991/results")
                ...
                scope.$watch(attr.benchmarkName, function (name) {
                    ...
                    if (sources.hasOwnProperty(name)) {
                        ...
                        var info = name.split(' ');
                        $http.post("http://localhost:8080/benchmarks/src", {
                            "benchmark": {
                                "name": info[0],
                                "version": info[1]
                            }
...
angular.module('app')
    .service('benchmarkData', function ($http, $q, Benchmark) {
        var benchmarks = [];
        // localhost:8080/performance
        return {
            getAll: function () {
                $http.get("http://localhost:8080/performance").success(function (data) {
                    var benchmarkByName = {};
